...
const CityProductPerCapitaPage: React.FC = () => {
  ...
  // New states for raw city product per capita and standardized score (for display and charts)
  const [actualCityProduct, setActualCityProduct] = useState<string | null>(null);
  ...
  const calculateAndSave = async () => {
    ...
    setActualCityProduct(cityProductPerCapitaValue.toFixed(2));
    ...
    const postData = {
      ...
      city_product_per_capita_standardized: parseFloat(standardizedValue.toFixed(2)),
  ...
  // Prepare pie data (standardized value + remainder)
    const standardizedScoreNumeric = standardizedScore ? parseFloat(standardizedScore) : 0;
    const pieData = [
      { name: 'Standardized', value: Number(standardizedScoreNumeric.toFixed(2)) },
      { name: 'Remaining', value: Number((100 - standardizedScoreNumeric).toFixed(2)) }
    ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50 flex items-center justify-center p-6">
      <div className="max-w-5xl w-full bg-white shadow-2xl rounded-2xl overflow-hidden">
        ...
        <div className="p-8">
          ...
          <div className="mb-4">
            ...
            {showSummary && (
              <div id="summary-panel" className="mt-3 p-4 bg-blue-50 border border-blue-100 rounded-md text-sm text-gray-700">
                <p className="mb-2">
                  City Product Per Capita measures the economic output per person in the city, adjusted for purchasing power parity (PPP).
                  This tool calculates the raw per capita product from the inputs you provide and converts it into a standardized 0–100 score using predefined benchmarks.
                </p>
                <p className="font-mono mb-2">City Product Per Capita (raw) = (Total City Product / Population) * PPP Rate PPP</p>
                <p className="mb-2">
                  Standardized score maps per capita product to 100..0 across benchmarks (MIN..MAX) so higher product → higher score.
                </p>

                <h4 className="font-semibold mt-2 mb-1">What to enter</h4>
                <ul className="list-disc list-inside mb-2">
                  <li>Enter national product and employment for each sector.</li>
                  <li>Enter city employment for each sector and total city population.</li>
                  <li>Enter the PPP exchange rate (e.g., 1 for USD).</li>
                ...
                <ul className="list-disc list-inside">
                  ...
                  <li>For comparisons, use data from reliable sources like national statistics.</li>
          ...
          {decision && actualCityProduct && standardizedScore && (
            <div className="mt-8 p-6 bg-gray-50 rounded-lg border">
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <h3 className="text-lg">Actual City Product Per Capita</h3>
                    <p className="text-2xl font-bold text-gray-800">
                      {actualCityProduct} PPP
                    </p>
                  </div>

                  <div>
                    <h3 className="text-lg">Standardized Product Score</h3>
                    <p className="text-2xl font-bold text-gray-800">
                      {standardizedScore}%
                    </p>
                  </div>
